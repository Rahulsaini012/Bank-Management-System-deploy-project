...
# MongoDB Connection
client = MongoClient("mongodb+srv://rahul123:rahulsaini2006@cluster0.dldqi.mongodb.net/?retryWrites=true&w=majority")
...
def create_account():
    ...
    if amount >= 1000 and pin.isdigit() and len(pin) == 4:
        ...
        return render_template('dashboard.html', user=customer, message="Account created successfully!")
    else:
        return render_template('index.html', error="Invalid input or minimum amount not met.")
...
def login():
    username = request.form['username']
    pin = request.form['pin']
    result = collection.find_one({"username": username, "pin": pin})
    if result:
        return render_template('dashboard.html', user=result)
    else:
        return render_template('index.html', error="Account not found or incorrect PIN.")
...
def credit():
    account_number = request.form['account_number']
    amount = float(request.form['amount'])
    collection.update_one({"account_number": account_number}, {"$inc": {"amount": amount}})
    return redirect('/')
...
def debit():
    account_number = request.form['account_number']
    amount = float(request.form['amount'])
    user = collection.find_one({"account_number": account_number})
    
    if user and user["amount"] >= amount:
        collection.update_one({"account_number": account_number}, {"$inc": {"amount": -amount}})
        return redirect('/')
    else:
        return "Insufficient balance!"
...
def delete():
    account_number = request.form['account_number']
    collection.delete_one({"account_number": account_number})
    return redirect('/')
